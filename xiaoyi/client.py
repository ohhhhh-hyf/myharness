...
from xiaoyi.serialization import (
...
class AnthropicClient(LLMClient):
    def __init__(self, config: ProviderConfig) -> None:
        ...
        if not api_key:
            raise AuthenticationError(
                ...
                "Set it in .xiaoyi/config.yaml or via ANTHROPIC_API_KEY env var."
            )
    ...
    async def stream(
        ...
        messages = build_anthropic_messages(conversation.get_messages(), vision=self._vision)
...
class OpenAIClient(LLMClient):
    def __init__(self, config: ProviderConfig) -> None:
        ...
        if not api_key:
            raise AuthenticationError(
                ...
                "Set it in .xiaoyi/config.yaml or via OPENAI_API_KEY env var."
            )
    ...
    async def stream(
        ...
        input_messages = build_openai_input(conversation.get_messages(), vision=self._vision)
...
class OpenAICompatClient(LLMClient):
    ...
    async def stream(
        ...
        messages = build_chat_completion_messages(conversation.get_messages(), vision=self._vision)

...
from xiaoyi.media import is_image_path
from xiaoyi.permissions.dangerous import DangerousCommandDetector, is_safe_command
from xiaoyi.permissions.modes import DecisionEffect, PermissionMode, mode_decide
from xiaoyi.permissions.rules import RuleEngine, extract_content
from xiaoyi.permissions.sandbox import PathSandbox
from xiaoyi.tools.base import Tool
...
class PermissionChecker:
    ...
    def check(self, tool: Tool, arguments: dict[str, Any]) -> Decision:
        ...
        # Layer 2: 路径沙箱（仅文件类工具）。
        # 例外：图片文件的只读访问不受沙箱限制——图片通常由用户从任意目录
        # （截图、下载目录等）提供，且只读不产生副作用。
        if tool.category in ("read", "write") and content:
            if not (tool.category == "read" and is_image_path(content)):
    ...
    def _is_plan_file(self, target_path: str) -> bool:
        if not self.plan_file_path or not target_path:
            return ".xiaoyi/plans/" in target_path
        try:

...
async def handle_permission(ctx: CommandContext) -> None:
    ...
    if sub == "":
    ...
    elif sub in _MODE_NAMES:
        # 直接切换：/permission acceptEdits，无需中间 mode 一词
        ctx.agent.set_permission_mode(_MODE_NAMES[sub])
        ctx.ui.refresh_status()
        ctx.ui.add_system_message(f"权限模式已切换为: {sub}")
        ...
        if not rule_str:
            ...
            return
        from xiaoyi.permissions.rules import Rule, parse_rule
        ...
        ctx.ui.add_system_message(
            "用法: /permission [模式 | rules | add <规则> | reset]\n"
            "模式可选: " + ", ".join(_MODE_NAMES.keys())
        )
...
PERMISSION_COMMAND = Command(
    name="permission",
    description="权限管理",
    usage="/permission [模式 | rules | add <规则> | reset]\n"
    "模式可选: " + ", ".join(_MODE_NAMES.keys()),

...
class ProviderConfig:
    ...
    api_key: str = ""
    thinking: bool = False
    # 是否支持图片输入（多模态）。默认开启；接入不支持图片的模型时用
    # vision: false 关闭，此时图片附件会降级为文字说明发送。
    vision: bool = True
...
def load_config(path: Path | None = None) -> AppConfig:
    ...
    home = Path.home()
    candidates = [
        home / ".xiaoyi" / "config.yaml",
        cwd / ".xiaoyi" / "config.yaml",
        cwd / ".xiaoyi" / "config.local.yaml",
    ]
    ...
    if merged is None:
        raise ConfigError(
            "No config file found. Expected .xiaoyi/config.yaml "
            "in project or ~/.xiaoyi/config.yaml"
        )
    return merged

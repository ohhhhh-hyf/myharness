...
from xiaoyi.hooks import HookConfigError, HookEngine, load_hooks
from xiaoyi.permissions import PermissionMode


def main() -> None:
    # 先确保 .xiaoyi/ 目录存在，否则下面写 debug.log 会因目录不存在而崩溃
    Path(".xiaoyi").mkdir(parents=True, exist_ok=True)
    logging.basicConfig(
        level=logging.INFO,
        format="%(asctime)s %(name)s %(message)s",
        filename=".xiaoyi/debug.log",
        filemode="w",
    )

    parser = argparse.ArgumentParser(prog="xiaoyi", description="XiaoYi AI coding assistant")
    parser.add_argument(
        "--mode",
    ...
    from xiaoyi.app import XiaoYiApp
    from xiaoyi.driver import NoAltScreenDriver
...
async def _run_prompt(config, permission_mode, hook_engine, prompt: str) -> None:
    ...
    from xiaoyi.client import create_client, resolve_context_window
    from xiaoyi.conversation import ConversationManager
    from xiaoyi.memory.instructions import load_instructions

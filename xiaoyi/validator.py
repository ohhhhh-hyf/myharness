...
def validate_providers(raw_providers: list) -> list[dict]:
    ...
    for i, entry in enumerate(raw_providers):
        ...
        # 视觉能力：默认视为支持（图片附件随请求直接发送，无需任何配置）；
        # 仅在接入不支持图片输入的模型时，用 vision: false 显式关闭。
        default_vision = True
        vision = entry.get("vision", default_vision)
        if not isinstance(vision, bool):
            raise ConfigError(f"Provider #{i + 1}: vision must be a boolean")

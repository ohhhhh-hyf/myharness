...
def _session_path(xiaoyi_dir: Path) -> Path:
    return xiaoyi_dir / SESSION_FILENAME
...
def load_worktree_session(xiaoyi_dir: Path) -> WorktreeSession | None:
    path = _session_path(xiaoyi_dir)
    if not path.exists():
        return None
    try:

...
from xiaoyi.worktree.models import Worktree, WorktreeSession
from xiaoyi.worktree.session import load_worktree_session, save_worktree_session
from xiaoyi.worktree.setup import perform_post_creation_setup
...
class WorktreeManager:
    def __init__(
        ...
        self.worktree_dir = worktree_dir or str(
            Path(repo_root) / ".xiaoyi" / "worktrees"
        )
        self._xiaoyi_dir = Path(repo_root) / ".xiaoyi"
    ...
    async def enter(self, name: str) -> WorktreeSession:
        ...
        save_worktree_session(self._xiaoyi_dir, session)
        return session
    ...
    async def exit(
        ...
        save_worktree_session(self._xiaoyi_dir, None)
    ...
    def restore_session(self) -> WorktreeSession | None:
        session = load_worktree_session(self._xiaoyi_dir)
        if session is None:
            return None

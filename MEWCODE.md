...
- 终端 UI：Textual（mewcode/app.py + styles.tcss，自定义 NoAltScreenDriver）
...
## 常用命令
- 装/同步依赖：`uv sync`
- 启动 TUI：`uv run mewcode`（读取 .mewcode/config.yaml）
- 单发模式：`uv run mewcode -p "问题"`
- 跑测试：`uv run pytest -q`

## 目录速览（mewcode/）
- agent.py 主循环与上下文管理；app.py TUI（MewCodeApp / ChatInput）；driver.py 终端驱动
- client.py 三个协议客户端；tools/ 工具注册与实现；permissions/ 权限矩阵与规则引擎
- mcp/ MCP 客户端与工具包装；agents/ 子智能体（内置 + .mewcode/agents/*.md）
...
## 注意事项
- .mewcode/ 是运行时状态目录（日志/会话/权限规则/含 API key 的配置），不要提交、不要当项目源码修改
